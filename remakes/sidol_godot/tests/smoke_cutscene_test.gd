...
func _ready() -> void:
	...
	# 데이터 무결성 선검증
	for path: String in ["res://data/cutscenes/opening.json", "res://data/maps/triggers_f1.json"]:
	...
	if typeof(mg_raw) != TYPE_DICTIONARY:
		...
		print(
			(
				"[smoke_cutscene] craft antibiotic_x=%d flag=%s"
				% [has_cure, GameState.has_flag("Q_F3_CURE_DONE")]
			)
		)
	...
	if (
		typeof(meta_raw) != TYPE_DICTIONARY
