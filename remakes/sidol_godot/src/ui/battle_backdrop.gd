...
## 전투 배경 — 벽면 + 지평선 + 원근 바닥.
##
## **정정(2026-09-07): "원작에는 전투 배경이 없었다"는 틀린 전제였다.**
## `WARMODE.C:1126` `RPut_Spr(0,0,&Back[0],0)` — `Back`은 `lth.spr`이고 그 프레임 0이
## 320×200 **어두운 학교 복도 배경**이다(공격 연출은 `sample2.spr`을 따로 깐다).
## 같은 파일의 프레임 1·2는 주인공의 평상·빈사 포즈다(`RPut_Spr(0,20,&Back[MELoss],0)`).
## `M-R-*.PCX`·`I-V-*.PCX`가 배경이 아니라 적 등장 일러스트인 것은 맞다 — 배경이 저기
## 없었을 뿐 원작에 배경 자체는 있었다.
##
## 그래도 이 파일을 원작 배경으로 갈아엎지는 않았다. 원작 배경은 **한 장 고정**이고
## 여기는 층별 톤 곡선(마스터 시나리오 §1.2)을 지고 있어서, 바꾸면 "지금 어디서 싸우는가"를
## 잃는다. 어느 쪽을 택할지는 유저 결정 사항으로 남긴다(백로그).
##
## 아래는 그 설계의 원래 목표 둘:
