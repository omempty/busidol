...
>
> **손으로 쓴 문서라 정본과 갈라졌다.** 2026-09-08 시점에 여기 적힌 캔버스 크기가
> 정본 계약과 달랐다 — `mad_eye`를 `320×512(셀 64)`로 적어 놨지만 정본은
> `640×1024(셀 128)`다. 게임 내 프레임 크기(64)를 **시트 셀 크기로 착각**한 것이고,
> 이 문서를 보고 만든 납품은 크기 위반으로 자동 반려됐다. 그래서 프롬프트를
> 손으로 적는 대신 스펙에서 굽는 `web_prompt.py`가 생겼다.
>
> **대신 쓸 것**(스펙에서 자동 생성 — 손으로 적지 않으므로 어긋나지 않는다):
>
> - 공통 규약: [`WEB_PROMPT_COMMON.md`](WEB_PROMPT_COMMON.md) — 대화 맨 앞에 한 번
> - 시트 상세: [`web/`](web/) 폴더의 `<카테고리>__<id>.md` (2026-09-09 실측 130개)
> - 셀 편집기의 **🌐 웹 챗용 프롬프트** 패널에서 버튼 하나로 복사할 수 있다
>   (`셀편집기.bat`)
> - 다시 굽기: `python tools/convert/web_prompt.py --write`
> - 사용법 전반: [`docs/TOOLS.md`](../../../docs/TOOLS.md)
>
> ### 2026-09-09에 한 일 — 틀린 수치를 걷어냈다
>
> 필드 몬스터 12종(`mad_eye`·`dworm`·`vulgar`·`ozzy`·`iron_voc`·`hellcop`·`o_ray`·
> `sparker`·`c_bug`·`flying_thesis`·`null_pointer`·`rogue_vending`)의 캔버스·셀 수치를
> **정본(`python tools/convert/web_prompt.py monsters <id>`)이 뱉는 값으로 바꿨다.**
> 열×행은 원래도 맞았고 **셀 크기만 64·48로 적혀 있었다** — 그래서 캔버스가 정확히
> 절반(또는 48/128)으로 줄어 있었다. 보스 7종은 원래도 정본과 같아 손대지 않았다.
>
> 첨부물 쪽이 오히려 맞았다: `_batch/0830_1133/monsters__<id>/grid_template.png`를
> 실측하니 12종 전부 정본 크기였다(예: `mad_eye` 640×1024). **문장만 틀렸던 것이다.**
> 예외 하나 — `sys_builder`의 `grid_template.png`는 1024×768이라 정본 1536×1152와
> 어긋난다(`python tools/review/prompt_audit.py`가 FAIL로 잡는다). 그 패키지는
> 다시 구워야 한다.
>
> 아래 본문은 **화풍 서술·영문 프롬프트 참고용**이다. 수치는 위 절차로 고쳤지만,
> 스펙이 또 바뀌면 여기는 자동으로 따라오지 않는다 — **의심되면 정본을 돌려라.**
...
## 📐 출력 규격 (그리드 계약 — 엄격 준수)
- **시트 캔버스 크기**: **640×1024px PNG** (셀 단위: 128×128px, 5열 × 8행) — 크기 정확 일치
...
2. 같은 폴더에서 아래 이미지를 함께 첨부합니다:
   - `1. grid_template.png` — 정확한 640×1024 규격의 캔버스 템플릿
...
## 📐 출력 규격 (그리드 계약 — 엄격 준수)
- **시트 캔버스 크기**: **512×1152px PNG** (셀 단위: 128×128px, 4열 × 9행) — 크기 정확 일치
- **행별 애니메이션 프레임 배치** (좌 ➔ 우가 재생 순서):
...
1. **ChatGPT-4o / Claude 3.5 Sonnet / Gemini Web** 채팅창에 위 본문을 **그대로 복사하여 붙여넣습니다**.
2. 같은 폴더에서 아래 이미지를 함께 첨부합니다:
   - `1. grid_template.png` — 정확한 512×1152 규격의 캔버스 템플릿
   - `2. style_ref.png` — 기존 게임 도트 화풍/정렬 기준
   - `3. subpalette.png` — 권장 컬러 팔레트 스왑치
   - `4. orig_enemy_1.png` — 원작/참조 콘셉트 이미지
3. 생성된 이미지를 다운로드하여 아래 경로에 저장합니다:
...
## 📐 출력 규격 (그리드 계약 — 엄격 준수)
- **시트 캔버스 크기**: **512×1024px PNG** (셀 단위: 128×128px, 4열 × 8행) — 크기 정확 일치
- **행별 애니메이션 프레임 배치** (좌 ➔ 우가 재생 순서):
...
1. **ChatGPT-4o / Claude 3.5 Sonnet / Gemini Web** 채팅창에 위 본문을 **그대로 복사하여 붙여넣습니다**.
2. 같은 폴더에서 아래 이미지를 함께 첨부합니다:
   - `1. grid_template.png` — 정확한 512×1024 규격의 캔버스 템플릿
   - `2. style_ref.png` — 기존 게임 도트 화풍/정렬 기준
   - `3. subpalette.png` — 권장 컬러 팔레트 스왑치
   - `4. orig_enemy_1.png` — 원작/참조 콘셉트 이미지
3. 생성된 이미지를 다운로드하여 아래 경로에 저장합니다:
...
## 📐 출력 규격 (그리드 계약 — 엄격 준수)
- **시트 캔버스 크기**: **768×1024px PNG** (셀 단위: 128×128px, 6열 × 8행) — 크기 정확 일치
- **행별 애니메이션 프레임 배치** (좌 ➔ 우가 재생 순서):
...
1. **ChatGPT-4o / Claude 3.5 Sonnet / Gemini Web** 채팅창에 위 본문을 **그대로 복사하여 붙여넣습니다**.
2. 같은 폴더에서 아래 이미지를 함께 첨부합니다:
   - `1. grid_template.png` — 정확한 768×1024 규격의 캔버스 템플릿
   - `2. style_ref.png` — 기존 게임 도트 화풍/정렬 기준
   - `3. subpalette.png` — 권장 컬러 팔레트 스왑치
   - `4. orig_enemy_1.png` — 원작/참조 콘셉트 이미지
3. 생성된 이미지를 다운로드하여 아래 경로에 저장합니다:
...
## 📐 출력 규격 (그리드 계약 — 엄격 준수)
- **시트 캔버스 크기**: **512×896px PNG** (셀 단위: 128×128px, 4열 × 7행) — 크기 정확 일치
- **행별 애니메이션 프레임 배치** (좌 ➔ 우가 재생 순서):
...
1. **ChatGPT-4o / Claude 3.5 Sonnet / Gemini Web** 채팅창에 위 본문을 **그대로 복사하여 붙여넣습니다**.
2. 같은 폴더에서 아래 이미지를 함께 첨부합니다:
   - `1. grid_template.png` — 정확한 512×896 규격의 캔버스 템플릿
   - `2. style_ref.png` — 기존 게임 도트 화풍/정렬 기준
   - `3. subpalette.png` — 권장 컬러 팔레트 스왑치
   - `4. orig_enemy_1.png` — 원작/참조 콘셉트 이미지
3. 생성된 이미지를 다운로드하여 아래 경로에 저장합니다:
...
## 📐 출력 규격 (그리드 계약 — 엄격 준수)
- **시트 캔버스 크기**: **512×384px PNG** (셀 단위: 128×128px, 4열 × 3행) — 크기 정확 일치
- **행별 애니메이션 프레임 배치** (좌 ➔ 우가 재생 순서):
...
1. **ChatGPT-4o / Claude 3.5 Sonnet / Gemini Web** 채팅창에 위 본문을 **그대로 복사하여 붙여넣습니다**.
2. 같은 폴더에서 아래 이미지를 함께 첨부합니다:
   - `1. grid_template.png` — 정확한 512×384 규격의 캔버스 템플릿
   - `2. style_ref.png` — 기존 게임 도트 화풍/정렬 기준
   - `3. subpalette.png` — 권장 컬러 팔레트 스왑치
   - `4. orig_enemy_1.png` — 원작/참조 콘셉트 이미지
3. 생성된 이미지를 다운로드하여 아래 경로에 저장합니다:
...
## 📐 출력 규격 (그리드 계약 — 엄격 준수)
- **시트 캔버스 크기**: **640×512px PNG** (셀 단위: 128×128px, 5열 × 4행) — 크기 정확 일치
- **행별 애니메이션 프레임 배치** (좌 ➔ 우가 재생 순서):
...
1. **ChatGPT-4o / Claude 3.5 Sonnet / Gemini Web** 채팅창에 위 본문을 **그대로 복사하여 붙여넣습니다**.
2. 같은 폴더에서 아래 이미지를 함께 첨부합니다:
   - `1. grid_template.png` — 정확한 640×512 규격의 캔버스 템플릿
   - `2. style_ref.png` — 기존 게임 도트 화풍/정렬 기준
   - `3. subpalette.png` — 권장 컬러 팔레트 스왑치
   - `4. orig_enemy_1.png` — 원작/참조 콘셉트 이미지
3. 생성된 이미지를 다운로드하여 아래 경로에 저장합니다:
