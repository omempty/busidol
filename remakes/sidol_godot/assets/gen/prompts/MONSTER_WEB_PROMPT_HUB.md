> ## ⚠️ 이 문서의 규격은 낡았다 (2026-09-08)
>
> 여기 적힌 캔버스 크기가 **정본 계약과 다르다**. 예: `mad_eye`를 `320×512(셀 64)`로
> 적어 놨지만 정본은 `640×1024(셀 128)`다 — 게임 내 프레임 크기(64)를 시트 셀 크기로
> 착각한 것이다. 이 문서를 보고 만든 납품은 크기 위반으로 자동 반려된다.
>
> **대신 쓸 것**(스펙에서 자동 생성 — 손으로 적지 않으므로 어긋나지 않는다):
>
> - 공통 규약: [`WEB_PROMPT_COMMON.md`](WEB_PROMPT_COMMON.md) — 대화 맨 앞에 한 번
> - 시트 상세: [`web/`](web/) 폴더의 `<카테고리>__<id>.md`
> - 셀 편집기의 **🌐 웹 챗용 프롬프트** 패널에서 버튼 하나로 복사할 수 있다
>   (`셀편집기.bat`)
> - 다시 굽기: `python tools/convert/web_prompt.py --write`
>
> 아래 본문은 화풍 서술·영문 프롬프트 참고용으로만 남긴다. **숫자는 믿지 마라.**
