# [요청] 화염 폭발 (flame_burst) — 전투 이펙트 시퀀스 1장

## 이 이펙트가 무엇인가 (이게 그림에 보여야 한다)
- 화염 비커가 깨지며 솟는 불기둥. 아래에서 위로 자라는 혀 모양 불꽃 + 흩어지는 불티. 흰-노랑 코어 / 주황 중간 / 진홍 외곽 3단 밴딩, 검은 연기 금지(남보라 연기)
- 속성: 화염
- 원작 참조 계열: fire

## 캔버스 — **1024×128px** (정확히)
**128×128px 칸을 8열 × 1행**으로 붙인 것이다
(128×8 = 1024, 128×1 = 128). 칸 경계선은 **그리지 않는다** — 보이지 않는
격자일 뿐이다.
...
## 이 시트에서 특히 자주 틀리는 것
1. 8칸이 사실상 같은 그림인 것(=애니메이션이 아니라 색 변주)
2. 이펙트를 맞는 대상까지 같이 그리는 것(대상은 게임이 따로 그린다)
3. 칸 경계에 선을 긋거나 프레임 번호를 써 넣는 것 → 반려
4. 반투명 알파로 페이드아웃시키는 것(반투명 픽셀 0%)
5. 캔버스 크기를 1024×128가 아닌 값으로 내보내는 것

## 내보내기 전 스스로 확인
- [ ] 캔버스가 정확히 1024×128px인가
- [ ] 8칸이 서로 **다른 단계**인가(형태·크기가 실제로 변하는가)
