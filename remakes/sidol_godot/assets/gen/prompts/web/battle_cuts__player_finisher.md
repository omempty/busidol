# [요청] 부싯돌의 불꽃 (player_finisher) — 전투 대형 연출 컷 1장
...
- 배우: player
- 재생 시점: 필살기(finisher_ember)

## 캔버스 — **2560×512px** (정확히)
**512×512px 칸을 5열 × 1행**으로 붙인 것이다
(512×5 = 2560, 512×1 = 512). 칸 경계선은 **그리지 않는다**.
...
## 리미티드 애니메이션 규칙
- 부드러운 중간 프레임 대신 **키 포즈의 대비**로 움직임을 만든다. 5장뿐이므로
  각 장이 결정적이어야 한다.
...
- [ ] 5칸의 포즈가 서로 확실히 다른가(같은 포즈의 미세 변주가 아닌가)
