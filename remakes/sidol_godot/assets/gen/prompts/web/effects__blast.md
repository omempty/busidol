# [요청] 대폭발 (blast) — 전투 이펙트 시퀀스 1장

## 이 이펙트가 무엇인가 (이게 그림에 보여야 한다)
- 필살기 착탄 폭발. 흰 섬광 → 팽창하는 화구 → 퍼지는 링과 파편 → 남보라 연기. 셀을 거의 꽉 채우는 크기까지 커진다

- 원작 참조 계열: mboom · rboom

## 캔버스 — **1280×128px** (정확히)
**128×128px 칸을 10열 × 1행**으로 붙인 것이다
(128×10 = 1280, 128×1 = 128). 칸 경계선은 **그리지 않는다** — 보이지 않는
격자일 뿐이다.

## 프레임 배치 (좌 → 우가 **시간 순서**)
| 행 | 시퀀스 | 프레임 수 | 시간 순서로 무엇이 일어나는가 |
|---|---|---|---|
| 0 | play | 10 | 섬광 → 화구 팽창 → 링 확산 → 연기 |
...
## 이 시트에서 특히 자주 틀리는 것
1. 10칸이 사실상 같은 그림인 것(=애니메이션이 아니라 색 변주)
2. 이펙트를 맞는 대상까지 같이 그리는 것(대상은 게임이 따로 그린다)
3. 칸 경계에 선을 긋거나 프레임 번호를 써 넣는 것 → 반려
4. 반투명 알파로 페이드아웃시키는 것(반투명 픽셀 0%)
5. 캔버스 크기를 1280×128가 아닌 값으로 내보내는 것

## 내보내기 전 스스로 확인
- [ ] 캔버스가 정확히 1280×128px인가
- [ ] 10칸이 서로 **다른 단계**인가(형태·크기가 실제로 변하는가)
