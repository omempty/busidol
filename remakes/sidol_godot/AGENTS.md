...
- **그림이 비면 늘 대체가 나온다**: 에셋은 데이터·코드보다 늦게 오는 것이 정상 순서다.
  미납품·미설치라고 화면을 비워 두지 않는다 — 키아트는 `assets/keyart/_fallback/`,
  초상은 `assets/portraits/_fallback.png`, 캐릭터 시트는 플레이어 시트로 떨어진다.
  **폴백이 있다고 미납품 신호를 지우지는 않는다**: `validate.gd`와 `portrait_check`는
  진짜 납품만 센다. 생성기: `tools/dev/make_keyart_fallback.py`.
