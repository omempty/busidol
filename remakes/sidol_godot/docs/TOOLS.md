...
### 지금 어디까지 배선됐나 — **통행 판정만 살아 있다**

`scenes/field.gd`가 부르는 것은 두 줄뿐이다(실측):

```gdscript
_props = PropsLayer.load_floor(GameState.current_floor)
_props.apply_to(runtime)          # ← attr_overrides()만 얹는다
```

즉 **소품은 지금 "보이지 않는 벽"이다.**

- ✅ **통행 판정** — 사물함 밑칸을 막고 윗칸으로는 지나간다.
- ❌ **그림** — `sprite` 필드는 읽히지 않는다. 타일셋 납품(§3.2)이 아직 없다.
- ❌ **조사(SPACE)** — `prop_at()`을 부르는 코드가 없다. `inspect.lines`·`requires_flag`가
  데이터에만 있다.

> 이 저장소의 **지배적 결함 유형이 바로 이것**이다(데이터는 있는데 코드가 안 읽는다).
> 여기서는 그것을 알고 남겨 둔 상태이므로, **다음에 손대는 사람은 `prop_at()` 배선과
> 스프라이트 렌더링부터 보면 된다.**
