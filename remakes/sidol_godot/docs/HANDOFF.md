...
> 작성일: 2026-08-28 (7차 세션 종료) · Phase 9 마감 트랙 — **P3 출하 블로커 해소**
...
**이제 남은 것은 대부분 "코드로 결정 불가"** — 폰트 선택(라이선스) · 맵 고립 구역이 원작인지
변환 산물인지 판정 · obj_id 173 재추출(원본 보유 PC) · 플레이스홀더 액터 8종(에셋 사이클).
마감 항목의 권위 목록은 [docs/03_plan/05_polish_roadmap.md](03_plan/05_polish_roadmap.md).

## 2. 다음 세션 첫 명령

```powershell
검증실행.bat                 # 관문 15단계 (14=world_audit, 15=내보내기 포함 규칙)
```
...
| 항목 | 왜 |
|---|---|
| obj_id 173 (WARN 2) | 원본(`originals/1995_sidol_bsd_dos/`)이 이 PC에 없다 — 재추출 불가 |
| 고립 구역·근거 없는 차단 (WARN 12) | 원작 맵과 대조해야 판정 가능. 원본 부재 |
| 플레이스홀더 액터·4방향 idle (WARN 7) | 에셋 납품 사이클 |
