...
> 대조하고 미이행 12건(P-A 3 + P-B 4 + P-C 3 + P-D 2)을 닫았다**
...
P-B(대화 로그 · 대사 속도 옵션 · 색각 패턴 · 레터박스)다. P-D의 두 건(전투 포트레이트 패널,
...
### 세션 끝에 남긴 미결 — f1 접촉 판정 신고 (유저 플레이 중 보고)

**결론을 못 냈다. 다음 세션이 여기서 이어야 한다.** 상세와 다음 단계는
[§5.11](03_plan/05_polish_roadmap.md).

* 「상자를 12시 방향으로 접근하면 반응 없다」 → **판정 코드에서는 결함을 못 찾았다.**
  f1 상자 27덩어리 전수 검사에서 못 여는 덩어리 0개, 대상이 판정 칸에 들어온 자리에서는
  성공률 100%. 원인은 **맵 기하**로 보인다 — 원본 맵에 상자가 벽에 끼어 특정 방향에서만
  열리는 자리가 실제로 있다(예: `(23,13)` 덩어리는 위·아래가 `ATT 1`이라 왼쪽에서만).
  맵은 동결이라 고치려면 **판정을 넓히는** 쪽이고, 그건 결정이 필요하다.
* 「시작 우측 방 NPC 2명 이벤트 없음」 → `npcs_f1.json`에 **NPC는 한 명뿐**(`tutor_dumb`
  12,11)이고 그 한 명은 정상 동작한다(프롬프트 「멍청 조교 SPACE」 캡처 확인).
  그 방의 두 인물은 NPC가 아니다. **가장 유력한 가설: f1 몬스터**(`vulgar`·`hellcop` 등은
  원작 도트가 사람 모습이라 NPC로 보인다). 확인 방법은 §5.11에 적었다.
