...
- **[미해결·중간] f0 계단 착지 불일치**: transitions.json의 대칭 오프셋(-3,-2)이
  f0 맵에서 벽을 가리킴((100,61) 불통행 — SelfCheck warn으로 상시 노출).
  원작 명세는 "모든 층 같은 좌표 공유"(04_game_systems §1.2)라 변환기
  (map_convert.py)의 f0 attr 직변환이 의심됨. 지하 콘텐츠 투입 전 변환기 점검
  필요. 현재는 디버그 패널 F0 이동 시 기본 스폰으로 폴백되어 접근 자체는 가능.
