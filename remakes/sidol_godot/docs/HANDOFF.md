...
### 3단계(전투 리듬·재미) — 진단만 하고 멈춤

다음 세션이 여기서 이어받는다. 고칠 것이 아니라 **아직 재지 않은 것**이 대부분이다:

1. **다중 적 대상 선택 불가** — 공격이 항상 `_first_alive_enemy()`. 약점 우선 처치라는
   선택지가 사라져 있다. UI(대상 커서) + 컨트롤러 타깃 인자가 함께 필요하다.
2. **도망 100% 성공** — 리스크가 없다. 원작 규칙은 `originals/.../WARMODE.C` 대조가 먼저다.
3. **턴 템포 미계측** — 스모크 실측으로 1회 액션 ≈ 1.0초(타이밍 링 포함)까지만 안다.
   층별 평균 턴 수·승률·브레이크 발생률을 재는 헤드리스 시뮬레이터가 없다.
