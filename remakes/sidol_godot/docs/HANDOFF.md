...
| 항목 | 근거 |
|---|---|
| **EXP 페이싱** | `growth.json`은 "각 층에서 자연히 1~2레벨업"이라 적어 두었는데 실측은 **레벨업당 20~35체**. 원작 요구 경험치(레벨당 50→80→110…)로는 f1 기준 7체다. `exp_accum`을 낮출지 `floor_stats.exp`를 올릴지는 밸런스 정책 결정 |
