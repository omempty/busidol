...
**다음 세션의 첫 결정거리**는 [§5.12](03_plan/05_polish_roadmap.md)의 **보류 8종**이다 —
원작 ATT 대화 마커 중 퀘스트 상태에 얽힌 것들을 원작 분기대로 살릴지, 지금 NPC·컷신
구현으로 볼지. 코드로 결정 불가다.
...
13종을 배선했다(맵 164칸) — `data/maps/talk_targets.json` + `TalkTargets` +
`field.gd` 조사 순서 한 단계(NPC 다음, 상자 앞) + `validate.gd` 양방향 대조.
두 벽 그림은 서로를 가리키고 다섯 번 참으면 「당신의 인내력에 감탄 했소.」가 나온다.
실습 조교는 진짜 힌트를 준다.

**보류 8종은 사람이 정해야 한다** — `16 Hong`·`21 Howang`·`51 Howa`·`56 HowaJo`
(화공과 퀘스트 `v_Howa` 분기) · `44 Q_Man`(퀴즈 별도 구현) · `66 SikDangGirl`(상점 op) ·
`77 ELIN`·`78 FELIN`(`v_Howa == 2`). 원작 분기를 그대로 살릴지, 지금 NPC·컷신 쪽
구현으로 볼지 충돌 가능성이 있다. **다음 세션의 첫 결정거리다.**
