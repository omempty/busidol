...
**다음 세션의 첫 결정거리**는 [§5.12](03_plan/05_polish_roadmap.md)의 **배치**다 —
제외한 3종(66 식당 아가씨 · 51 화공과 교수 · 44 퀴즈맨)은 원작 좌표와 리메이크 NPC
좌표가 다르다. 원작 자리로 옮길지, 지금 자리를 유지할지는 코드로 결정 불가다.
...
**제외 3종**: `66 SikDangGirl`(NPC `cafeteria_girl`이 바로 아래 칸에 서 있고 상점까지
담당) · `51 Howa`(NPC `prof_chem`이 같은 인물) · `44 Q_Man`(퀴즈는 `f3_quiz_gate`로
이미 구현). **남은 판단은 배치다** — 원작 좌표와 리메이크 NPC 좌표가 달라, 원작 자리로
옮길지는 사람이 정해야 한다.
