...
#### SPACE 한 번에 여러 대상이 걸릴 때 — **우선순위 정본**

판정 셀은 `InteractProbe`가 단일 소스다(정면 2칸이 먼저, 어깨 2칸이 나중).
같은 칸에 여러 대상이 겹칠 수 있으므로 순서가 곧 규칙이다.
`scenes/field.gd`의 `_physics_process`가 이 순서로 `return` 한다:

```
interact 트리거 → 계단(빠른 이동) → NPC → 워커 → 원작 ATT 마커 → 상자 → 소품
```

**규칙은 "원작에 있던 것이 먼저"다.** 마커·상자는 원작 데이터에 있던 대상이고,
소품은 리메이크가 얹은 덧층이라 맨 뒤다. 트리거가 맨 앞인 이유는 따로 있다 —
계단 앵커 위의 컷신(폭파·희생)이 빠른 이동에 가로막히면 진행이 영영 막힌다
(2026-09-05 실측: 방문층 2개부터 blast 불발).

알약(프롬프트)은 하나뿐이므로 이 순서가 그대로 "무엇을 안내하는가"가 된다.
이벤트가 아직 안 난 소품 앞에서는 소품 이름이 알약에 뜨고, 누르면 트리거가 이긴다 —
즉 소품이 트리거에게 **없던 시각 단서를 준다**.
