...
| 콘텐츠 | 툴 | 형태 | 근거 |
|---|---|---|---|
| 맵·타일 | ~~**Tiled 1.9+**~~ **미도입 — 불필요로 확정(2026-08-30)** | — | 오쏘고널 그리드 RPG 표준이나 **이 프로젝트에서는 맵 지형이 편집 대상이 아니다.** 관문 19단계 `originals_check`가 맵 234,000셀의 원본 바이트 일치를 지키고, [04_uiux §0](04_uiux_modernization.md)이 "큰 문맥(맵 지형)은 원작을 따른다"로 못 박았다. 실제로 `.tmj`/`.tmx` 파일 0개 · Tiled 임포터 0개이며, 맵 파이프라인은 `tools/convert/map_convert.py`(원본 → JSON) **단방향**뿐이다. 편집이 필요한 것은 지형이 아니라 그 위에 얹는 배치 데이터(`triggers_f*`·`npcs_f*`·`walkers_f*`·`transitions`)이고, 그쪽은 §3.3의 좌표 피커가 담당한다([05_polish_roadmap §5.10 #40](../03_plan/05_polish_roadmap.md) — 조건부 보류) |
